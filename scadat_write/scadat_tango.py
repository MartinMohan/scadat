...
import os,sys,re,argparse,socket,string
import PyTango,scadat_version
#def usg(name=None):                                                            
#    return '''[-v] [-h] [ -l | -r R [R ...] | -a A [A] | -c C [C] | -p P [P]]'''

#def myhelp(name=None):                                                            
#        return '''Usage: [-h] [-v] [-t T] [ -l | -r R [R ...] | -a A [A] | -c C [C] | -p P [P]]
def myhelp(name=None):                                                            
        return '''Usage: [-h] [-v] [-t T] [ -l | -r R [R ...] | -w W [W] ]
...
optional arguments:
  ...
  -w W [W]      Write attributes,commands and properties for devices: e.g.
                scadat -w sys/tg_test/1/myproperty test
                scadat -w sys/tg_test/1/string_scalar 'hello world'
                scadat -w sys/tg_test/1/DevLong 1234'''
...
#            ,formatter_class=argparse.RawTextHelpFormatter)
    parser = argparse.ArgumentParser(description=\
            "Linux command line interface to tango (default: TANGO_HOST=127.0.0.1:10000)\n"
            ,formatter_class=argparse.RawTextHelpFormatter,usage='scadat'+myhelp(),add_help=False)
...
#    parser.add_argument("-t", default="127.0.0.1:10000",
    parser.add_argument("-t",
            help="Set TANGO_HOST - default 127.0.0.1:10000")
    ...
    group.add_argument("-r",nargs='+',
            ...
            "scadat -r sys/tg_test/1\n")
    group.add_argument("-w",nargs='+',
            help="Write attributes,commands and properties for devices: e.g.\n"
                "scadat -w sys/tg_test/1/myproperty test"
                "scadat -w sys/tg_test/1/string_scalar 'hello world'"
                "scadat -w sys/tg_test/1/DevLong 1234")
...
#  -w W [W]      Write attributes,commands and properties for devices: e.g.
    group.add_argument("-a",nargs='+',
            help="Read/Write attributes of a device: e.g.\n"
            "scadat -a sys/tg_test/1\n"
            "scadat -a sys/tg_test/1/string_scalar 'hello world'")
    group.add_argument("-c",nargs='+',
            help="Read/Send commands to a device: e.g.\n"
            "scadat -c sys/tg_test/1 -v\n"
            "scadat -c sys/tg_test/1/DevLong 1234\n")
    group.add_argument("-p",nargs='+',
            help="Read/Write properties to a device: e.g.\n"
    ...
    if len(sys.argv)==1:
#        print ("scadat -h: For usage\nscadat -v: For info")
#        sys.exit(1)
        args.h=1
    return args 

def main():
    """ Call scadat commands from command line """
    args=parse_input()
#    if args.t:
#        print ("args.t %s"%args.t)
    if not os.getenv("SUPER_TANGO"):
        os.environ["SUPER_TANGO"] = "true"

    myscadat=scadat_tango(verbose=args.v,TANGO_HOST=args.t)
...
#        print ('\n- TO BE DONE')
        if len(args.w)==1:
            print ('\n'.join(myscadat.attrib(args.w[0])))
        elif len(args.w)==2:
            print ('\n'.join(myscadat.attrib(args.w[0],args.w[1])))
        else:
            print ("Usage: scadat -w sys/tg_test/1/attr_name <value>")
    elif args.a:
        if len(args.a)==1:
...
#            print (myscadat.cmd(args.c[0],args.c[1]))
        else:
...
#                print ("arg %s deviceName %s attrName %s"%(arg,deviceName,attrName))
                cmdType="Invalid"
                if attrName:
                    try:
                        cmdType="attr"
...
#                        print ('\n'.alist)
        ...
        myscadat.version_info()
#        print(myscadat.version_info())
#        print ('\n'.join(myscadat.info()))
...
class scadat_tango():
    ...
    def __init__(self,verbose=0,TANGO_HOST=None):
        """set os.environ['TANGO_HOST'] to point to machine required
        If TANGO_HOST not set use 127.0.0.1:10000
        set verbose level to 0,1,2,3"""
        self.verbose=verbose # global verbose level
        if TANGO_HOST:
            os.environ["TANGO_HOST"]=TANGO_HOST
        else:
            os.environ["TANGO_HOST"]=os.getenv("TANGO_HOST", "127.0.0.1:10000")
...
#        return True
    ...
    def version_info(self):
        ...
        str_list = []
#        print("===============scadat HOST===============")
#        print("local host="socket.gethostname() +" ("+socket.gethostbyname(socket.gethostname())+" )")
        print("scadat_version "+str(scadat_version.version))
        print("python.version_info "+str(sys.version))
        print("PyTango.__version_ "+PyTango.__version__)
        ...
        if self.verbose>=2:
            try:
                ...
                from PyTango.utils import info
                print (info())
#                print (Pytango.utils.info())
            except:
                print("os.platform - fail")
...
#        print("===============scadat TARGET===============")
        ...
        try:
            ...
            DeviceHandle=PyTango.DeviceProxy(starter_device)
...
#            str_list.append("Cannot open Starter");
        ...
        try:
            tango_lib_version=str(DeviceHandle.get_tango_lib_version()) # >PyTango 8.1.0
            if self.verbose>=2:
...
-vv         -All exported devices and associated classnames"""                
        ...
        for mydev in dev_list:
            ...
            try:
                dev_info=self.db.get_device_info(mydev) #Only works PyTango >= 8.1.0
                if self.verbose>=2:
...
#        """Read all tango attributes/properties and commands"""
#        deviceName,attrName=self.split_device_name(InputName)
#        if(attrName==None):
#            return(self.rattrib(InputName)+self.rcmd(InputName)+self.rprop(InputName))
#        else:
#            sys.stderr.write("Invalid name %s (expected aa/bb/cc)\n"%(InputName))
#            return([]) #empty list
    ...
    def rattrib(self,InputName):
        ...
        deviceName,attrName=self.split_device_name(InputName)
        try:
            DeviceHandle = PyTango.DeviceProxy(deviceName)
        except Exception as e:
...
#                print ("cmd_type.in_type %s arg %s"%(cmd_type.in_type,arg))
    ...
    def rprop(self,InputName):
        ...
        -bug in PyTango db.get_device_list() does not read default values sourceforge feature request 121 """                
        str_list = []
        deviceName,attrName=self.split_device_name(InputName)
...
#                        str_list.append("%s: %s %s"%(lattrName,keys,values))
    ...
    def prop(self,InputName,arg=None):
        """Read/Write tango device property attrName/propValue
...
#                mydict2=PyTangoDbDatum(attrname)
...
#
    ...
    def get_starter_devices(self):
        ...
        try:
            ...
            print ("get_device_exported_for_class Starter -Fail. Try -v")
            sys.exit(0)
        DeviceHandle = PyTango.DeviceProxy(starter_device)
        RunningServers=DeviceHandle.read_attribute("RunningServers").value
        if RunningServers==None:
            print "Starter RunningServers %s. Try -v\n"%str(RunningServers)
            sys.exit(0)
...
#        return(str_list)
    ...
    def getInputType(self,cmd_type,value):
        ...
        e.g. PyTango.DevFloat to float.
        Otherwise command will not work?"""
#        print ("1. In:getInputType cmd_type==%s,value=%s"%(cmd_type,value))

        if cmd_type == PyTango.DevFloat:
            return float(value)
        elif cmd_type == PyTango.DevDouble:
            return float(value)
        elif cmd_type == PyTango.DevLong:
#            print("In PyTango.DevLong cmd_type %s, value %s ,type(value) %s"%(cmd_type,value,type(value)))
            return int(value)
        elif cmd_type == PyTango.DevLong64:
            return int(value)
        elif cmd_type == PyTango.DevULong64:
            return int(value)
        elif cmd_type == PyTango.DevULong:
            return int(value)
        elif cmd_type == PyTango.DevInt:
            return int(value)
        elif cmd_type == PyTango.DevBoolean:
            if value==str(0):
                return False
            else:
                return True
        elif cmd_type == PyTango.DevShort:
            return int(value)
        elif cmd_type == PyTango.DevUShort:
            return int(value)
        elif cmd_type == PyTango.DevString:
            return str(value)
        elif cmd_type == PyTango.DevVarLongArray:
...
#            return map(int, value.split())
        ...
        elif cmd_type == PyTango.DevVarLong64Array:
            return list(map(int, value.split()))
        elif cmd_type == PyTango.DevVarULong64Array:
            return list(map(int, value.split()))
        elif cmd_type == PyTango.DevVarFloatArray:
            return list(map(float, value.split()))
        elif cmd_type == PyTango.DevVarDoubleArray:
            return list(map(float, value.split()))
        elif cmd_type == PyTango.DevVarShortArray:
            return list(map(int, value.split()))
        elif cmd_type == PyTango.DevVarUShortArray:
            return list(map(int, value.split()))
        elif cmd_type == PyTango.DevVarStringArray:
            return list(map(str, value.split()))
        elif cmd_type == PyTango.DevVoid:
            return None
# Not sure about stuff below
        elif cmd_type == PyTango.DevVarDoubleStringArray:
            return "PyTango.DevVarDoubleStringArray:Not tested yet"
#            return map(str, value.split())
        elif cmd_type == PyTango.DevVarCharArray:
#           "1 2 3 4" -> [1 2 3 4]
            return "PyTango.DevVarCharArray:Not tested yet"
        elif cmd_type == PyTango.DevVarLongStringArray:
            return "PyTango.DevVarLongStringArray:Not tested yet"
        elif cmd_type == PyTango.DevState:
            return "PyTango.DevVarLongStringArray:Not tested yet"
        else:
